...
def load_csv_to_oracle(csv_path, table_name):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            cursor.execute(f"""
                INSERT INTO {table_name} (id, vendor_id, account, company_code, amount, period, status)
                VALUES (:1, :2, :3, :4, :5, TO_DATE(:6, 'YYYY-MM'), :7)
            """, (
                int(row["id"]),
                row["vendor_id"],
                row["account"],
                ...
                row["status"][:5] if pd.notnull(row["status"]) else None
            ))
            inserted += 1
...
if __name__ == "__main__":
    ...
    csv_file = os.path.join(base_dir, "data", "ap_entries.csv")
    ...
    load_csv_to_oracle(csv_file, "ap_entries")